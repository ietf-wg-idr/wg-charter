...
The primary objective (and priority) of the Inter-Domain Routing (IDR) Working Group (WG) is to develop and maintain BGP as the standard inter-domain routing protocol deployed for IPv4 and IPv6 routing over the Internet.  IDR WG will develop and maintain BGP features, extensions, and mechanisms that are largely address-family independent and may be considered core to the protocol’s operation. These include:
...
* Various "community" technologies, including: BGP Communities \[RFC1997\], Extended Communities \[RFC4360\] and others, Large BGP Communities \[RFC 8092\], and Wide Communities, for carrying metadata and policy-related information.  
...
* Operation and optimization of Route Reflectors \[RFC4456\] and Confederations \[RFC5065\] for scalable BGP deployments.  
* Outbound Route Filtering (ORF) mechanisms \[RFC5291\] and its extensions for policy-based control of route advertisement.  
* Graceful Restart procedures, including  \[RFC4724\]  and Long-Lived Graceful Restart\[RFC9494\] for improving BGP session resiliency and convergence during restarts or failures.
...
**1) Infrastructure (Underlay) Routing:** These address families support the routing of infrastructure prefixes, commonly referred to as underlay routing:
...
**2) Routing-Adjacent Information Dissemination:** These address families enable BGP to carry information related to, or adjacent to, routing functionality:
...
* Development of YANG data models scoped to IDR chartered technologies to support the management of BGP features standardized by the WG, with a focus on device models and capturing operational considerations, as appropriate.  Providing advice to other BGP-related working groups with development of their own BGP-related YANG models for technologies under their charter.
...
As listed in the work areas, the IDR WG may take up work on BGP extensions to initiate and support chartered and adopted use cases happening in other WGs following consultation with the relevant WG Chairs and Responsible ADs. The IDR WG shall coordinate closely with the originating WG(s) responsible for the overall framework, architecture, and requirements of newly proposed or updated BGP extensions. 

Work on technologies related to such chartered and adopted use cases in other WGs is not in the purview of IDR.  Once relevant BGP extensions have been defined to support the use cases, the relevant WG should be able to make use of the new extensions with a high degree of autonomy and not require the work to be done primarily in IDR.  IDR, or appropriate review designees such as the BGP directorate, will provide review of the use of such new extensions upon request by the relevant WG's chairs or ADs with the goal of ensuring correct use of such features and providing stewardship of shared technologies.  An example of such a shared technology is the Tunnel Encapsulation Attribute \[RFC9012\]. Relevant WGs include, but are not limited to: SPRING, TEAS, CATS, SAVNET, and, in the context of BGP-LS, LSR.
